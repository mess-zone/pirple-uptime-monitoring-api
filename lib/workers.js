...
workers.log = (originalCheckData, checkOutcome, state, alertWarranted, timeOfCheck) => {
    ...
    _logs.append(logFileName, logString, (err)=>{
        if(!err) {
            debug('loggin in file succeded')
        } else {
            debug('loggin in file failed')
        }
    })
}
...
workers.gatherAllChecks = () => {
    ...
    _data.list('checks', (err, checks) => {
        if(!err && checks && checks.length > 0) {
            ...
            debug('error: could not found checks to process')
        }
    })
}
...
workers.validateCheckData = (originalCheckData)=> {
    ...
    if(originalCheckData.id &&
        ...
        debug('one of the checks are not properly formated. Skipping it')
    }
}
...
workers.alertUserToStatusChange = (checkData) => {
    ...
    helpers.sendTwilioSms(checkData.userPhone, msg, (err)=> {
        if(!err) {
            debug('user was alerted: ', msg)
        } else {
            debug('could not send sms: ', msg)
        }
    })
}
